...
// 월간 핵심 지표를 아이콘, 값, 보조 설명이 있는 카드로 보여줍니다.
...
// 작은 차트 영역의 제목, 설명, 차트 본문을 공통 레이아웃으로 묶습니다.
...
// 생산성/영양 흐름처럼 단순 수치 배열을 막대 그래프로 표시합니다.
...
// 선택 기간의 체중 변화를 간단한 선 그래프로 표시합니다.
export function WeightLine({ values }: { values: number[] }) {
  if (values.length < 2) {
    return (
